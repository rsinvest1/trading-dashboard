...
import { pnlByDayOfWeek } from '../../utils/analytics';
...
export default function DayOfWeekChart() {
  ...
  const data = useMemo(() => pnlByDayOfWeek(trades), [trades]);

  if (!data.some(d => d.count > 0)) return null;

  return (
    <div className="card p-4 h-72">
      <h3 className="text-sm uppercase tracking-wider text-text-secondary mb-3">P&L by Day of Week</h3>
      <ResponsiveContainer width="100%" height="85%">
        <BarChart data={data} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
          <XAxis dataKey="day" stroke="#9ca3af" fontSize={11} />
          <YAxis tickFormatter={v => fmtMoney(v).replace('$', '')} stroke="#6b7280" fontSize={10} />
